...
describe('held item runtime positioning', () => {
    ...
    it('retreats held item from solids without moving astronaut position', () => {
        const heldCollectable = { x: 0, y: 0, velocity: { x: 0, y: 0 }, setHeldFacing: vi.fn() };
        const astronaut = { position: { x: 100, y: 120 }, velocity: { x: 0, y: 0 } };
        const collidesAtSide = vi.fn((x: number) => x >= 113);
        const runtime = createGameHeldItemRuntime({
            astronaut,
            movementSettings: {
                heldCollectableVerticalOffset: -6,
        ...
        expect(astronaut.position.x).toBe(100);
